...
export default function HomePage() {
  ...
  useEffect(() => {
    (async () => {
      const [best, latest, sale, cats] = await Promise.all([
  ...
  return (
    <div className="animate-fade-in">
      {/* Preload hero images */}
      {heroSlides.map((s, i) => (
        <link key={i} rel="preload" as="image" href={s.image} fetchpriority={i === 0 ? 'high' : 'low'} />
      ))}
      {/* Hero Slider */}
      <section className="relative h-[500px] sm:h-[600px] lg:h-[680px] overflow-hidden gpu">
        {heroSlides.map((s, i) => (
          ...
          >
            ...
            <OptimizedImage src={s.image} alt={s.title} priority={i === 0} loading={i === 0 ? 'eager' : 'lazy'} className="w-full h-full object-cover" />
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {categories.filter(c => !SHORTCUT_CATEGORIES.some(sc => sc.slug === c.slug)).map((cat) => (
            <button
              key={cat.id}
            ...
            >
              <OptimizedImage
                src={categoryIcons[cat.slug] || cat.image_url || 'https://images.pexels.com/photos/8617715/pexels-photo-8617715.jpeg'}
                alt={cat.name}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 text-center">
                <h3 className="font-prompt text-lg font-bold text-white">{cat.name}</h3>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 cv-auto">
        <div className="relative rounded-3xl overflow-hidden bg-gradient-to-r from-rose-200 via-accent to-rose-100 p-8 lg:p-16">
          ...
          <div className="absolute right-0 top-0 bottom-0 w-1/2 opacity-30 hidden lg:block">
            <OptimizedImage src="https://images.pexels.com/photos/8617718/pexels-photo-8617718.jpeg" alt="" className="w-full h-full object-cover" />
          </div>
        </div>
      </section>
