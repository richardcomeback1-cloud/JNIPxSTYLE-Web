  ...
  useEffect(() => {
    (async () => {
      ...
      if (categorySlug && !isShortcut) {
        ...
        if (cat) {
          // category_id ของสินค้าคือ "หมวดหมู่หลัก" เสมอ ส่วนประเภท (เลือกได้หลายรายการ)
          // เก็บอยู่ใน category_ids — ถ้าลูกค้ากดชิป "ประเภท" ให้กรองเพิ่มด้วย category_ids
          ...
          if (subCategory) {
            const { data: subCat } = await supabase
              ...
              .eq('name', subCategory)
              .maybeSingle();
            if (subCat) query = query.contains('category_ids', [subCat.id]);
          }
        }
