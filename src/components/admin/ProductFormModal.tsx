...
export default function ProductFormModal({ product, categories, onClose, onSave }: Props) {
  ...
  // หมวดหมู่หลัก (ไม่มี parent) กับ "ประเภท" (มี parent_id) — category_id ใช้เก็บ
  // หมวดหมู่หลักอย่างเดียวเสมอ ส่วน category_ids เก็บรายการประเภทที่เลือกได้หลายรายการ
  const rootCategories = useMemo(() => categories.filter((c) => !c.parent_id), [categories]);

  const [selectedRootId, setSelectedRootId] = useState(() => product?.category_id || '');
  ...
  const handleRootCategoryChange = (rootId: string) => {
    ...
    // เปลี่ยนหมวดหมู่หลัก = รีเซ็ตประเภทที่เคยเลือกไว้ (ประเภทเดิมอาจไม่มีอยู่ในหมวดใหม่)
    setForm((f) => ({ ...f, category_id: rootId, category_ids: [] }));
  };

  const toggleType = (subId: string) => {
    setForm((f) => ({
      ...f,
      category_ids: f.category_ids.includes(subId)
        ? f.category_ids.filter((id) => id !== subId)
        : [...f.category_ids, subId],
    }));
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="col-span-2">
              <label className={labelClass}>ประเภท (เลือกได้มากกว่า 1)</label>
              {!selectedRootId ? (
                <p className="text-sm text-taupe-300 py-2.5">เลือกหมวดหมู่ก่อน</p>
                    ...
                    return (
                      <label
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-sm cursor-pointer transition-colors ${
                          checked ? 'bg-rose-500 text-white border-rose-500' : 'bg-white text-taupe-500 border-rose-200 hover:border-rose-400'
                        }`}
                      >
                        <input
                          type="checkbox"
