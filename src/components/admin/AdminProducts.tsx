...
export default function AdminProducts({ products, categories, onProductsChange }: Props) {
  ...
  // แสดง "หมวดหมู่หลัก › ประเภท1, ประเภท2" ต่อสินค้าหนึ่งชิ้น (ประเภทเลือกได้หลายรายการ)
  ...
  const categoryLabel = (product: Product) => {
    ...
    const root = categories.find((c) => c.id === product.category_id);
    if (!root) return null;
    const typeNames = (product.category_ids || [])
      .map((id) => categories.find((c) => c.id === id)?.name)
      .filter((n): n is string => Boolean(n));
    return typeNames.length > 0 ? `${root.name} › ${typeNames.join(', ')}` : root.name;
  };
            ...
            <tbody>
              {pageItems.map((p) => (
                <tr key={p.id} className="border-t border-rose-50 hover:bg-cream/50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    {categoryLabel(p) ? (
                      <span className="text-xs text-taupe-500 bg-cream px-2 py-1 rounded-full whitespace-nowrap">{categoryLabel(p)}</span>
                    ) : (
