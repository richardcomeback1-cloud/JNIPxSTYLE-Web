...
export default function AdminProducts({ products, categories, onProductsChange }: Props) {
  ...
  // แสดง "หมวดหมู่หลัก > ประเภทย่อย" ต่อสินค้าหนึ่งชิ้น (หรือแค่หมวดหลักถ้าไม่มีประเภทย่อย)
  // เพื่อให้แอดมินเห็นการจัดหมวดหมู่ของสินค้าแต่ละชิ้นได้ทันทีในตาราง ไม่ต้องเปิดฟอร์มแก้ไขทีละชิ้น
  ...
  // ตัวเลือกหมวดหมู่หลักในตัวกรอง เรียงตาม sort_order เหมือนหน้าจัดการหมวดหมู่
  ...
  const filtered = useMemo(() => {
    ...
    let result = products;
    if (q) result = result.filter((p) => p.name.toLowerCase().includes(q) || p.slug.includes(q));
    if (categoryFilter) {
      // นับสินค้าที่อยู่ในหมวดหลักนี้ตรง ๆ หรืออยู่ในประเภทย่อยใด ๆ ของหมวดหลักนี้
      const matchIds = new Set(
        categories.filter((c) => c.id === categoryFilter || c.parent_id === categoryFilter).map((c) => c.id)
      );
      result = result.filter((p) => p.category_id && matchIds.has(p.category_id));
    }
    return result;
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-4">
        ...
        <div className="flex gap-2">
          ...
          <select
            ...
            className="px-3 py-2.5 border border-rose-200 rounded-full bg-white text-sm text-taupe-500 focus:outline-none focus:border-rose-400"
          >
          ...
          <table className="w-full">
            <thead className="bg-cream">
              <tr className="text-left text-xs text-taupe-400 uppercase tracking-wider">
                ...
                <th className="px-4 py-3">หมวดหมู่ / ประเภท</th>
                ...
                <tr key={p.id} className="border-t border-rose-50 hover:bg-cream/50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    {categoryLabel(p.category_id) ? (
                      <span className="text-xs text-taupe-500 bg-cream px-2 py-1 rounded-full whitespace-nowrap">{categoryLabel(p.category_id)}</span>
                    ) : (
                      <span className="text-xs text-rose-400">ยังไม่ระบุ</span>
                    )}
                  </td>
                  <td className="px-4 py-3">
