...
export default function Header() {
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50 bg-cream border-b border-rose-200/50">
                  ...
                  {suggestions.length > 0 && (
                    <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-xl border border-rose-100 overflow-hidden z-10">
                      {suggestions.map((p) => (
                        ...
                        >
                          <OptimizedImage src={(p.images?.[0]) || ''} alt={p.name} width={80} className="w-10 h-10 rounded-lg object-cover shrink-0" />
